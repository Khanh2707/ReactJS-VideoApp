...
export default function DetailVideo() {
  ...
  const [openDialogConfirmDeleteVideo, setOpenDialogConfirmDeleteVideo] =
    useState(false);
  const [openDialogLockVideo, setOpenDialogLockVideo] = useState(false);
  const [openBackdropCheckVideoDisplay, setOpenBackdropCheckVideoDisplay] =
    useState(true);
  const [videoIsBan, setVideoIsBan] = useState(true);
  const [displayWhenVideoCant, setDisplayWhenVideoCant] = useState(false);
  ...
  const handleLikeVideo = () => {
    videoAPI
      ...
      .then((response) => {
        ...
        handleOpenSnackbar(
          "success",
          `Bạn vừa like video ${video?.title}`,
          "bottom",
          "center"
        );
      })
  ...
  const handleUnLikeVideo = () => {
    videoAPI
      ...
      .then((response) => {
        ...
        handleOpenSnackbar(
          "info",
          `Bạn vừa hủy like video ${video?.title}`,
          "bottom",
          "center"
        );
      })
  ...
  const handleDownloadVideo = () => {
    handleOpenSnackbar("info", "Video sắp được tải", "bottom", "center");
    videoAPI
  ...
  const handleUpdateLockVideo = async () => {
    try {
      handleOpenSnackbar("info", "Video đang bị khóa!", "bottom", "center");

      const updateResponse = await videoAPI.updateVideo(idVideo, {
        hide: false,
        ban: true,
      });

      const countResponse = await videoAPI.countVideoIsBan(
        video?.channel.idChannel
      );
      let countVideoIsBan = countResponse.result;

      if (countVideoIsBan >= 2) {
        const banChannelResponse = await channelAPI.createBanChannel({
          idChannel: video?.channel.idChannel,
        });
      }
  ...
  useEffect(() => {
    if (video?.ban !== null && video?.ban === false && video?.hide === false) {
      setOpenBackdropCheckVideoDisplay(false);
      setVideoIsBan(false);
    } else if (
      video?.ban !== null &&
      video?.ban === false &&
